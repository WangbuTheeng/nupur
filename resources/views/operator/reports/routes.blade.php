...
@section('title', 'Route Performance Report')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-gradient-to-r from-green-600 to-green-800 overflow-hidden shadow-xl rounded-xl mb-8">
        <div class="px-6 py-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Route Performance Report</h1>
                    <p class="text-green-100">Analyze route profitability and passenger demand</p>
                </div>
    ...
    <div class="bg-white overflow-hidden shadow-lg rounded-xl mb-8">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('operator.reports.routes') }}" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
    ...
    <div class="bg-white overflow-hidden shadow-lg rounded-xl">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Route Performance Data</h3>
        ...
        <div class="p-6">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">
                                                    {{ $routeData['route']->sourceCity->name }} → {{ $routeData['route']->destinationCity->name }}
                                                </div>
                                                <div class="text-sm text-gray-500">{{ $routeData['route']->distance ?? 'N/A' }} km</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $routeData['total_schedules'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $routeData['total_bookings'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $routeData['total_passengers'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rs. {{ number_format($routeData['total_revenue'], 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-1 bg-gray-200 rounded-full h-2 mr-2">
                                                <div class="bg-green-600 h-2 rounded-full" style="width: {{ min($routeData['occupancy_rate'], 100) }}%"></div>
                                            </div>
                                            <span class="text-sm text-gray-900">{{ number_format($routeData['occupancy_rate'], 1) }}%</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rs. {{ number_format($routeData['avg_revenue_per_trip'], 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="mt-1 text-sm text-gray-500">No route performance data available for the selected period.</p>
                </div>
            @endif
        </div>
    </div>
